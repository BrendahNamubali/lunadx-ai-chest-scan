...
import { SCREENING_OPTIONS, getScreeningCopy, normalizeScreeningMode, type ScreeningMode } from "@/lib/screening";
...
export default function UploadPage() {
  ...
  const [screeningMode, setScreeningMode] = useState<ScreeningMode>("pneumonia");

  const screeningCopy = getScreeningCopy(screeningMode);
  ...
  function buildScan(aiResponse: Record<string, unknown>, patient: { id: string; name: string }, imageUrl: string): ScanResult {
    const mode = normalizeScreeningMode(
      (aiResponse._screeningMode as string | undefined) ?? screeningMode
    );
    const sim = simulateAI(mode);
    const tbScore = mode === "tuberculosis" ? (aiResponse.tb_probability as number | undefined) ?? sim.tbRisk : 0;
    const pnaScore =
      mode === "pneumonia" ? (aiResponse.pneumonia_probability as number | undefined) ?? sim.pneumoniaRisk : 0;
    const primary = mode === "tuberculosis" ? tbScore : pnaScore;
    ...
    return {
      ...
      imageUrl,
      screeningMode: mode,
      tbRisk: Math.round(tbScore),
      pneumoniaRisk: Math.round(pnaScore),
      lungOpacityRisk: mode === "pneumonia" ? (aiResponse._lungOpacityRisk as number | undefined) ?? sim.lungOpacityRisk : 0,
      pleuralEffusionRisk: mode === "pneumonia" ? (aiResponse._pleuralEffusionRisk as number | undefined) ?? sim.pleuralEffusionRisk : 0,
      lungNodulesRisk: mode === "pneumonia" ? (aiResponse._lungNodulesRisk as number | undefined) ?? sim.lungNodulesRisk : 0,
      abnormalityScore: Math.min(Math.round(primary), 100),
      riskLevel: primary > 70 ? "High" : primary > 40 ? "Medium" : "Low",
      findings: (aiResponse._findings as string[] | undefined) ?? sim.findings,
      suggestions: (aiResponse._suggestions as string[] | undefined) ?? sim.suggestions,
      aiSummary: aiResponse.ai_summary as string | undefined,
      heatmapOverlayUrl: (aiResponse.heatmap_overlay_url as string | null) || undefined,
  ...
  const handleTrySample = async () => {
    ...
    try {
      const aiResponse = await analyzeXray(
        "/sample-xray.jpg",
        demoPatient.id,
        "Sample case — persistent cough",
        "PA",
        screeningMode
      );
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const aiResponse = await analyzeXray(
        preview!,
        patientId,
        clinicalNotes || patient.symptoms,
        viewPosition,
        screeningMode
      );
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground mb-6 sm:mb-8">{screeningCopy.uploadSubtitle}</p>

      <div className="space-y-6">
        <div>
          <Label>Screening Type</Label>
          <Select value={screeningMode} onValueChange={(v) => setScreeningMode(v as ScreeningMode)}>
          ...
          <p className="text-xs text-muted-foreground mt-1.5">
            {screeningMode === "tuberculosis"
              ? "Uses the Hades-Hellix TB model (Hugging Face) — separate from pneumonia inference."
              : "Uses the dedicated pneumonia ViT model — TB model is not run on this path."}
          </p>
        </div>
        {/* Patient */}
        <div>
        ...
        >
          {analyzing
            ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> {screeningCopy.analyzingLabel}</span>
            : <span className="flex items-center gap-2"><FileImage className="w-4 h-4" /> {screeningCopy.analyzeButton}</span>}
        </Button>
      </div>
